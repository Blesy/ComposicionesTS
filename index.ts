...
fs.writeFileSync('./src/salida/data.json', '[');
for (let i = 0; i < largo - 8; i++) 
{
    for (let j = i+1; j < largo - 7; j++) 
    {
        for (let k = j+1; k < largo - 6; k++) 
        {
            for (let l = k+1; l < largo - 5; l++) 
            {
                for (let m = l+1; m < largo - 4; m++) 
                {
                    for (let n = m+1; n < largo - 3; n++) 
                    {
                        for (let o = n+1; o < largo - 2; o++) 
                        {
                            for (let p = o+1; p < largo - 1; p++) 
                            {
                                for (let q = p + 1; q < largo; q++) 
                                {
                                    limpiar();
                                    let composicion: Heroes[] = componer(i, j, k, l, m, n, o, p, q);
                                    sumar(composicion);
                                    if (sumaRasgos > 18){
...
function escribir(cadena: string){
    let path = fs.realpathSync('./src/salida/data.json');
    fs.appendFileSync(path, `${cadena} \n`);
}
...
function componer(i: number, j: number, k: number, l: number, m: number, n: number, o: number, p: number, q: number) {
    let listaContador: number[] = [i, j, k, l, m, n, o, p, q];
    let composicion: Heroes[] = [null, null, null, null, null, null, null, null, null];
